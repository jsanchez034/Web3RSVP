...
function getOrCreateAccount(address: Address): Account {
  ...
  if (account === null) {
    ...
    account.totalAttendeeEvents = integer.ZERO;
    account.save();
  }
...
export function handleNewRSVP(event: NewRSVP): void {
  const id = event.params.eventId.toHex() + event.params.attendeeAdress.toHex();
  let newRSVP = RSVP.load(id);
  const thisEvent = Event.load(event.params.eventId.toHex());
  
  if (newRSVP === null && thisEvent !== null) {
    const account = getOrCreateAccount(event.params.attendeeAdress);
...
export function handleConfirmedAttendee(event: ConfirmedAttendee): void {
  const id = event.params.eventId.toHex() + event.params.attendeeAddress.toHex();
  ...
  if (newConfirmation === null && thisEvent !== null) {
    ...
    account.totalAttendeeEvents = integer.increment(account.totalAttendeeEvents);
    account.save();
  }
}
